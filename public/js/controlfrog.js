...
function updatetemp() {
    $.getJSON("api/device?key="+temp, 
      function(json){
        ...
        var temp = $('#temp').next();
        var change = json.data.temp[1];
        if(change > 0) {
        	temp.removeClass("m-green").addClass("m-red");
        	temp.find('div').removeClass("arrow-down").addClass("arrow-up");
        }else{
        	temp.removeClass("m-red").addClass( "m-green" );
        	temp.find('div').removeClass("arrow-up").addClass("arrow-down");
        }
        temp.find('span').text( Math.abs(change)  + '%');
    });
}
    ...
    $.getJSON("api/device?key="+humidity, 
      function(json){
        ...
        if(change > 0) {
        	...
        	humidity.removeClass("m-red").addClass( "m-green" );
        	humidity.find('div').removeClass("arrow-up").addClass("arrow-down");
        }
        humidity.find('span').text( Math.abs(change)  + '%');
    });
}
